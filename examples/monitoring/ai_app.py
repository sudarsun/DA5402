...
info.info({'version': '0.0.13', 'buildhost': '@resonance', 'author': 'Dr. Su Sa', 'builddate': 'March 2026'})
...
@api_usage.time()
def make_cpu_busy(time_secs:int, request:Request):

    host = "unknown"
    if request.client is not None:
        log.info(f"make_cpu_busy: received request to make cpu busy for {time_secs} seconds from client {request.client.host}")
        host = request.client.host
    
    # increment the counter per host per api.
    counter.labels(endpoint='/make_cpu_busy', client=host).inc()
...
def extract_np(data:Data, request:Request, lang:str="en"):
    ...
    host = "unknown"
    if request.client is not None:
        log.info(f"extract_np: received request from client {request.client.host}")
        host = request.client.host

    # let's track the amount of data processed by this API.
    request_size.observe(amount=len(data.text))
    # increment the counter per host per api.
    counter.labels(endpoint='/np', client=host).inc()
...
def extract_ne(data:Data, request:Request):
    ...
    host = "unknown"
    if request.client is not None:
        log.info(f"extract_ne: received request from client {request.client.host}")
        host = request.client.host

    # let's track the amount of data processed by this API.
    request_size.observe(amount=len(data.text))
    # increment the counter per host per api.
    counter.labels(endpoint='/ne', client=host).inc()
...
@app.post("/nptext")
@api_usage.time()
def extract_body(request:Request, text:str=Body(...)):  # the ... (ellipses) operator is a placeholder.
    ...
    host = "unknown"
    if request.client is not None:
        log.info(f"extract_body: received request from client {request.client.host}")
        host = request.client.host
        
    counter.labels(endpoint='/nptext', client=host).inc()
    lines = text.split("\n")
    records = []
    for line in lines:
        doc_en = nlp_en(line)
