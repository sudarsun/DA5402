...
file_task = FileSensor(task_id='check_file', filepath='/opt/airflow/dags/files/employees.csv')

@task
def push(ti=None):
    """Pushes an XCom without a specific target"""
    ti.xcom_push(key="value from pusher 1", value=value_1)
    df = pd.read_csv('/opt/airflow/dags/files/employees.csv') 
    ti.xcom_push(key="dataframe", value = df)

@task
def filter_by_leave(ti = None):
    df = ti.xcom_pull(task_ids="push", key="dataframe")
    df = df[df.Leave != 0]
    return df

def report_df_size(ti = None):
    df = ti.xcom_pull(task_ids="filter_by_leave", key="return_value")
    return df.shape

report_task = PythonOperator(task_id = "report", python_callable=report_df_size)
...
with DAG(
    ...
    puller(push_by_returning()) << push() << file_task
    ...
    #p1 >> f1 >> r1
    #report_df_size() << filter_by_leave() << push()

    #report_task << filter_by_leave() << push() << file_task

    file_task >> push() >> filter_by_leave() >> report_task
    # >>  >> 
